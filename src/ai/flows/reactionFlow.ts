...
const prompt = ai.definePrompt({
  ...
  **Your Task:**
  ...
  4.  **Dangers and Warnings**: A clear, direct summary of the potential dangers and hazards. Analyze the reactants and products to describe specific risks like toxicity, corrosiveness, flammability, or violent reactions.
  ...
  12. **Flame Test**: Describe what happens if the products are exposed to a high-temperature flame, including flame color and reactivity.
  13. **Freeze Test**: Describe what happens if the products are rapidly frozen. Your description MUST be influenced by the 'freezeSpeed' input (e.g., 'rapid' freezing leads to smaller crystals).
  14. **Total Cost**: You MUST use the 'getChemicalPrice' tool for each reactant, passing the current 'grade'. Sum the prices to get the total cost. Format the final number with commas as thousand separators (e.g., "$1,234,567.89"). If the number is extremely large (e.g., over a billion), you may use suffixes (e.g., "$1.2 Billion").
  15. **Element I/O**: Analyze the elemental composition of all reactants and products. Provide two arrays for input and output elements and their total relative amounts. This MUST be an accurate accounting of all atoms.
