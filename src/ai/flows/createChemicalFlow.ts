...
const prompt = ai.definePrompt({
  ...
  2.  **Process by the Final Category**:
      -   If the final category is **'ordinary'**: The user wants a single, simple chemical. The user may provide a properly formatted chemical formula with subscripts (e.g., H₂SO₄). Use the 'name' to find its properties using the getChemicalProperties tool. If not found by common name, use the provided name as the formula if it looks like one.
      ...
      -   If the final category is **'custom'**: The user is inventing an item. Use the 'name' and 'description' to be creative. Generate a plausible but fictional scientific name, a cool common name, and a unique formula (e.g., 'Xy-7b'). The formula should be based on the provided name, even if it uses special characters. The effects should match the description. For example, if the description says "a glowing, unstable rock," generate high glow and explosion values. **Do not simplify the user's concept.** For example, "Aerated Water" should not become "Water". It should be treated as a unique substance.
